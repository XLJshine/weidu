...
@implementation ChooseViewfirstTableView

- (id)initWithFrame:(CGRect)frame titleArray:(NSArray *)array  block:(BlockCellSelect)block{
    if (self) {
        self = [super initWithFrame:frame];
        self.backgroundColor = [UIColor colorWithWhite:0.9059 alpha:1];
        _block = block;
...
- (void)tableReloadData:(NSArray *)titleArray  frame:(CGRect)rect   block:(BlockCellSelect)block{
    //self.frame = rect;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.contentView.backgroundColor = [UIColor colorWithWhite:0.9059 alpha:1];
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 135, 33)];
