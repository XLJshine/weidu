...
@implementation RenmaiTongxunTableViewCell
- (void)layoutSubviews{
...
- (void)addButton:(NSInteger)tag{
    ...
    [_tianjiaButton setImage:[UIImage imageNamed:@"renmaiyaoqing@2x"] forState:UIControlStateNormal];
...
- (void)tianjiaButtonAction:(UIButton *)button{
    ...
    if ([_delegate respondsToSelector:@selector(TongxuntableViewCell:index:)]) {
        [_delegate TongxuntableViewCell:self  index:button.tag];
    }
}
- (void)awakeFromNib {
    // Initialization code
