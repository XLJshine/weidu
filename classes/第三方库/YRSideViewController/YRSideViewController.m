...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _leftViewShowWidth = 258;
        _rightViewShowWidth = 258;
...
- (void)viewDidLoad
{
    ...
    self.needSwipeShowMenu = false;    //bu允许左右滑动*******************
    //监听，隐藏阴影
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(hideviewDismiss) name:@"hideviewDismiss" object:nil];
    
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(SideViewShow) name:@"xianshicehua" object:nil];
}
...
- (void)showShadow:(BOOL)show{
    ...
    _currentView.layer.shadowOpacity    = show ? 0.5f : 0.0f;
    if (show) {
...
- (void)willShowLeftViewController{
    ...
    self.needSwipeShowMenu = true;    //允许左右滑动*******************
}
- (void)willShowRightViewController{
    ...
    self.needSwipeShowMenu = false;    //不允许左右滑动*******************
}
...
//    /*平移带缩放效果的动画
    ...
    //scale = MAX(0.8, scale);  //原始效果
    scale = MAX(1, scale);      //jimmy修改效果
