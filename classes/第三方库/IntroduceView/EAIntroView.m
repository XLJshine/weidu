...
- (void)buildScrollViewWithFrame:(CGRect)frame {
    ...
    //添加pageControl
    pageControl1 = [[UIPageControl alloc]initWithFrame:CGRectMake((self.bounds.size.width - 100)*0.5, self.bounds.size.height * 0.85, 100, 10)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (page == (pageViews.count - 1) && self.swipeToExit) {
        ...
        self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width*3 + (self.bounds.size.width - 270)*0.5, self.bounds.size.height * 0.8 - 60, 270, 42)];
        //[self.skipButton setAutoresizingMask: UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin];
        [self.skipButton setImage:[UIImage imageNamed:@"getInView@2x"] forState:UIControlStateNormal];
    ...
    //NSLog(@"x = %f",x);
    //int page1 = (self.bounds.size.width * 0.5 + x)/self.bounds.size.width + 1;
