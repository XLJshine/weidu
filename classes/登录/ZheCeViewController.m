...
@interface ZheCeViewController ()

@end

@implementation ZheCeViewController{
    NSInteger t;
    NSTimer*_myTimer;
    NSString* _str;
   ...
   _myTimer= [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(action) userInfo:nil repeats:YES];
    ...
    for (int i = 0; i< _areaArray.count; i++)
    {
        ...
        if ([code1 isEqualToString:[@"+86" stringByReplacingOccurrencesOfString:@"+" withString:@""]]) {
            ...
            BOOL isMatch = [pred evaluateWithObject:_phoneNum.text];
            if (!isMatch)
            {
                //手机号码不正确
            ...
            if (_phoneNum.text.length>0&&_name.text.length>0&&_mima.text.length>0&&_yanzhengma.text.length>0&&[_mima.text isEqualToString:_mimaAgain.text]) {
                ...
                [manager GET:urlString parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
                    ...
                    if ([code isEqualToString:@"0"]) {
                        ...
                        [userDic setValue:_phoneNum.text forKey:@"userName"];
                        ...
                        //调用登录接口
                        NSString*urlString=[NSString stringWithFormat:@"%@user/login?account=%@&password=%@",ApiUrlHead,_phoneNum.text,_mima.text];
                        NSLog(@"urlString = %@",urlString);
                        AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
                        [manager GET:urlString parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
                            NSLog(@"JSON: %@", responseObject);
