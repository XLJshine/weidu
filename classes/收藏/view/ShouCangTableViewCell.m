...
-(void)addControl{
    ...
    [vDeleteButton setBackgroundColor:[UIColor redColor]];
    ...
    [vMoreButton setTitle:@"分享" forState:UIControlStateNormal];
...
- (void)layoutSubviews{
    ...
    vMenuView.frame =CGRectMake(self.frame.size.width - DELETE_BUTTON_WIDHT, 0, DELETE_BUTTON_WIDHT, self.frame.size.height);
    ...
    vDeleteButton.frame = CGRectMake(0, 0, DELETE_BUTTON_WIDHT, self.frame.size.height);
...
#pragma mark 点击删除
-(void)deleteButtonClicked:(id)aSender{
    NSLog(@"点击删除");
    [self.superview sendSubviewToBack:self];
    if ([_delegate respondsToSelector:@selector(didCellClickedDeleteButton:index:)]) {
        [_delegate didCellClickedDeleteButton:self index:self.tag];
    }
}
